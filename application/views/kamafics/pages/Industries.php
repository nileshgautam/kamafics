        <!-- CONTENT START -->
        <div class="page-content">
            <!-- INNER PAGE BANNER -->
            <div class="wt-bnr-inr overlay-wraper bg-center" style="background-image:url(<?php echo base_url('assets/images/banner/3.jpg')?>);">
                <div class="overlay-main site-bg-secondry opacity-09"></div>
                <div class="container">
                    <div class="wt-bnr-inr-entry">
                        <div class="banner-title-outer">
                            <div class="banner-title-name">
                                <h2 class="site-text-primary">Industry</h2>
                            </div>
                        </div>
                        <!-- BREADCRUMB ROW -->

                        <div>
                            <ul class="wt-breadcrumb breadcrumb-style-2">
                                <li><a href="javascript:void(0);">Home</a></li>
                                <li>Industry</li>
                            </ul>
                        </div>

                        <!-- BREADCRUMB ROW END -->
                    </div>
                </div>
            </div>
            <!-- INNER PAGE BANNER END -->
            <!-- ALL SERVICES START -->
            <div class="section-full p-t80 p-b40 bg-no-repeat bg-bottom-right bg-cover" style="background-image:url(<?php echo base_url('assets/images/background/bg-7.jpg') ?>)">
                <div class="container">
                    <div class="section-content">
                        <!-- TITLE START-->
                        <div class="section-head center wt-small-separator-outer text-center">
                            <div class="wt-small-separator site-text-primary">
                                <div class="sep-leaf-left"></div>
                                <div>The Best Industry services</div>
                                <div class="sep-leaf-right"></div>
                            </div>
                            <h2>KMAFICS India during its tenure of 30+ years, boasts of a clientele from the following Industries</h2>
                        </div>
                        <!-- TITLE END-->
                        <div class="row d-flex justify-content-center">
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">

                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Language Services
                                            </span>
                                            <!-- <span class="s-title-two">Engineering</span> -->
                                        </div>
                                        <div class="service-box-content">
                                            <p>Language instruction, cross cultural training, translation services and publishing products</p>

                                        </div>



                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">

                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Packing</span>

                                        </div>
                                        <div class="service-box-content">
                                            <p>Design and Manufacturing of strapping machines, strips, banders, crushers, accessories and consumables
                                            </p>
                                        </div>



                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">

                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Weather
                                            </span>
                                            <span class="s-title-two">Services</span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>Weather Forecasting Company
                                            </p>

                                        </div>



                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">

                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Metallurgical
                                            </span>

                                        </div>
                                        <div class="service-box-content">
                                            <p>Manufacturer of Machinery for the metallurgical industry</p>
                                        </div>



                                    </div>
                                </div>
                            </div>
                           


                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">

                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Solar
                                            </span>
                                            <!-- <span class="s-title-two">Engineering</span> -->
                                        </div>
                                        <div class="service-box-content">
                                            <p>Construction of solar fields for thermoelectric plants
                                            </p>

                                        </div>



                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">

                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Sheet Metal </span>
                                            <span class="s-title-two">Solutions</span>

                                        </div>
                                        <div class="service-box-content">
                                            <p>Sheet metal solutions for the wind energy sector, railways, electrical equipment and automotive sector.
                                            </p>
                                        </div>



                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">

                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Telecommunication

                                            </span>
                                            <!-- <span class="s-title-two">Services</span> -->
                                        </div>
                                        <div class="service-box-content">
                                            <p>Telecommunication, Information Technology and Consumer Electronics mainly designing and manufacturing mobile telephones

                                            </p>

                                        </div>



                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">

                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Moulds and Parts

                                            </span>

                                        </div>
                                        <div class="service-box-content">
                                            <p>
                                                <ol>
                                                    <li>Designing and Manufacturing Moulds</li>
                                                    <li>Designing and Manufacturing Thermoformed Plastic Parts
                                                    </li>
                                                </ol>
                                            </p>
                                        </div>



                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Valves
                                            </span>
                                            <!-- <span class="s-title-two">Equipment</span> -->
                                        </div>
                                        <div class="service-box-content">
                                            <p>Designing, manufacturing of knife gate valves, penstocks, dampers and valves for hydraulic work.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Stone</span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>
                                                Heat retaining soapstone fireplaces, sauna and interior.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Foundary
                                            </span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>
                                                Manufacturing castings in carbon steel, stainless steel, higher nickel alloys, tools steel, martensitic steel and other special steels.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Sports
                                            </span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>
                                                Design, Manufacturer, Distrubutor and Retailer of Sporting Equipment mainly for the sport Tennis.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Tobacco
                                            </span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>
                                                Manufacturer and distribution of Tobacco and producing and selling cigarettes.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Lifting </span>
                                            <span class="s-title-two">Equipment</span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>
                                                Design, Manufacture and Service of Overhead Cranes.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Cold Storage Systems</span>
                                            <span class="s-title-two">Storage Systems</span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>
                                                Design, Manufacture and Set Up of cold storage units and cold storage transport units
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Musical
                                            </span>
                                            <span class="s-title-two">Equipment</span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>Design and Manufacturing of amplifiers, guitars and its accessories, pianos and electric pianos, chimes, flutes, banjos, mandolin, ukuleles, percussion, baffles, mixing console, microphone and its accessories, accordions, harmonicas, electric batteries, dishes, drumsticks, pads, trumpets, saxophones, violin and hard&soft covers.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 m-b50">
                                <div class="service-border-box">
                                    <div class="wt-box service-box-1 bg-white">
                                        <div class="service-box-title title-style-2 site-text-secondry">
                                            <span class="s-title-one">Automotive</span>
                                        </div>
                                        <div class="service-box-content">
                                            <p>
                                                Manufacturers of Keys, Key Manufacturing Machines and Key Duplication Software & Machine. Manufacturers of Transponder Keys and Remote Controls , Manufacturer of Motorcycles, Scooters, Mopeds and Recreational All Terrain Vehicles.
                                            </p>
                                            <p>
                                                Design, Manufacture and Entry of Race Cars for Formula One Championships . Design, Testing, Engineering and Homolgation Services to the Automotive Industry] Design, Testing and Manufacturing of braking systems for 2W and 3W. Automotive Lighting Manufacture for 2W, 4W and Commercial Vehicles
                                                Torsion and Tension Spring Manufacturers.
                                            </p>

                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>




        </div>
        <!-- CONTENT END -->